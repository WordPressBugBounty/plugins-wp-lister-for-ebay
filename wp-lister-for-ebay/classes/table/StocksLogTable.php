 ...
 */
class WPLE_StocksLogTable extends WP_List_Table {

    const TABLENAME = 'ebay_stocks_log';
    var $total_items;
    ...
    function __construct(){
        ...
        parent::__construct( array(
            ...
            'screen'    => 'ebay_page_wplister-stockslog'
        ) );
    ...
    function column_product_id($item){
        ...
        if ( is_object($product) && wple_get_product_meta( $product, 'product_type' ) == 'variation' ) {
            if ( is_callable( array( $product, 'get_parent_id' ) ) ) {
                $post_id = $product->get_parent_id();
                $title .= ' ('. $post_id .')';
            } else {
                $post_id = wple_get_product_meta( $product, 'id' );
                $title .= ' (' . $post_id . ')';
            }
        }
    ...
    public function get_columns(){
        $columns = array(
            ...
            'product_id'        => __( 'Product ID', 'wp-lister-for-ebay' ),
            'sku'               => __( 'SKU', 'wp-lister-for-ebay' ),
            'old_stock'         => __( 'Old Stock', 'wp-lister-for-ebay' ),
            'new_stock'         => __( 'New Stock', 'wp-lister-for-ebay' ),
            'caller'            => __( 'Caller', 'wp-lister-for-ebay' ),
            'method'			=> __( 'Method', 'wp-lister-for-ebay' ),
    ...
    function getPageItems( $current_page, $per_page ) {
        ...
        if ( isset( $_REQUEST['usertype'] ) && !empty($_REQUEST['usertype']) ) {
            $usertype = wple_clean($_REQUEST['usertype']);
    ...
    public function get_table_size() {
        ...
        if ( $wpdb->last_error ) echo 'Error in get_table_size(): '.$wpdb->last_error;
